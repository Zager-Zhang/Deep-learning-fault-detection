    ...
    def train(self, writer):
        ...
        for epoch1 in range(self.start_epoch, args.max_epoch):
            ...
            for phase in ['train', 'val']:
                ...
                writer.add_scalar(f"epoch_loss_{phase}", epoch_loss, epoch1 + 1)
                writer.add_scalar(f"epoch_acc_{phase}", epoch_acc, epoch1 + 1)
                writer.add_scalar(f"epoch_precision_{phase}", epoch_precision, epoch1 + 1)
                writer.add_scalar(f"epoch_recall_{phase}", epoch_recall, epoch1 + 1)
                writer.add_scalar(f"epoch_false_alarm_{phase}", epoch_false_alarm, epoch1 + 1)
                writer.add_scalar(f"epoch_miss_rate_{phase}", epoch_miss_rate, epoch1 + 1)
                writer.add_scalar(f"epoch_F1_{phase}", F1, epoch1 + 1)
